...
holding the one's complement of the type byte. Some are stateless actions the unit
itself keeps the state for, which is why the remote sends the same bytes every time.
...
class FujitsuAcProtocol(IntEnum):
    ...
    across 18-30 C with STANDARD frames and reported every setpoint back correctly
    through its wireless LAN adapter. So either family reaches the right temperature.

    What the choice does decide is resolution, and risk. Only EXTENDED can express a
    half degree. Only STANDARD is known to be understood by every unit, since nothing
    has tested whether a unit old enough to predate EXTENDED ignores its protocol byte.
    """
...
# The families scale the field differently and agree only at 24 C. Both units span the
# same field range, 16 to 44, which is 16-30 C or 60-88 F.
_TEMPERATURE_SCALES = {
...
def temperature_scale(
    protocol: FujitsuAcProtocol, *, fahrenheit: bool = False
) -> _TemperatureScale:
    """Return how a protocol reads its temperature field in the given unit."""
    scale = _TEMPERATURE_SCALES.get((protocol, fahrenheit))
    if scale is None:
        raise ValueError(f"{protocol.name} remotes have no Fahrenheit setting")
    return scale


def temperature_step(protocol: FujitsuAcProtocol, *, fahrenheit: bool = False) -> float:
    """Return the smallest temperature change a protocol's remote can express."""
    return temperature_scale(protocol, fahrenheit=fahrenheit).step
...
def _util_message(message_type: int) -> bytes:
    """Build a 7-byte util message of the given type."""
    message = [*_COMMON_SIGNATURE, message_type, 0]
    message[_UTIL_LENGTH - 1] = _util_checksum(message)
...
def _state_message(
    *,
    ...
    _set_field(message, _FAHRENHEIT, fahrenheit)
    _set_field(
        message,
        _TEMPERATURE,
        _encode_temperature(
            temperature_scale(protocol, fahrenheit=fahrenheit), temperature
        ),
    )
...
class _FujitsuAcMessage(AehaCommand):
    """Shared physical layer for the two Fujitsu General message kinds."""

    TIMING = _FUJITSU_TIMING
...
class FujitsuAcCommand(_FujitsuAcMessage):
    ...
    ``power`` is not an on/off switch but the flag that starts a unit which is
    currently off. A remote sets it only while the unit is off; a state message
    without it changes the settings of a running unit and leaves a stopped one
    stopped. Stopping a unit is a fixed code, not a state message.

    ``protocol`` selects the frame format. It does not have to match the remote the
    unit came with, since a unit reads the format out of the frame, but only
    ``EXTENDED`` can carry a half degree and only ``STANDARD`` is known to be
    understood by every unit.

    ``temperature`` is in degrees Celsius unless ``fahrenheit`` is set, which only an
    ``EXTENDED`` remote can express. Both units use the same field, so a unit reads
    whichever one the flag names.

    ``clean`` is the clean function on most remotes. On an ``EXTENDED`` remote it is
    also how the 10 C minimum-heat function is sent, combined with
    :attr:`FujitsuAcMode.FAN_ONLY`.
    """
    ...
    def from_raw_timings(cls, timings: list[int]) -> Self | None:
        ...
        Returns a FujitsuAcCommand if the timings carry a state message, or None
        otherwise. A util message carries no state, so it decodes to None here and to
        a :class:`FujitsuAcFixedCommand` instead.
        """
        ...
        fahrenheit = bool(_get_field(message, _FAHRENHEIT))
        try:
            scale = temperature_scale(protocol, fahrenheit=fahrenheit)
        except ValueError:
            return None
...
class FujitsuAcFixedCommand(_FujitsuAcMessage):
    ...
    @classmethod
    def from_raw_timings(cls, timings: list[int]) -> Self | None:
        """Decode raw IR timings into a FujitsuAcFixedCommand.

        Returns any valid util message as its type byte, including types this library
        does not name. A state message decodes to None here; use ``FujitsuAcCommand``
        to read one as a mode, temperature, fan and swing instead.
        """
        message = cls._common_message(timings)
